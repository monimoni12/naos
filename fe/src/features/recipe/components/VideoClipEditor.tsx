...
/**
 * VideoClipEditor - 스크립트 클립 편집기
 * Lovable VideoClipEditor.tsx → Next.js 마이그레이션
 * 
 * 스크립트 텍스트를 클릭해서 클립 분할 위치를 지정
 */
...
interface VideoClipEditorProps {
  ...
  scriptText: string;
  onClose: () => void;
  onSave?: (splitPositions: number[]) => void;
}
...
export default function VideoClipEditor({ 
        ...
        parts.push(
          <span key={`split-${i}`} className="inline-flex items-center mx-1">
            <button
              ...
              className="inline-flex items-center gap-1 px-2 py-0.5 bg-[#FF6B35]/20 text-[#FF6B35] rounded hover:bg-[#FF6B35]/30 transition-colors text-xs font-medium"
            >
  ...
  const handleSave = () => {
    ...
    savedClips[recipeId] = splitPositions;
    localStorage.setItem("videoClips", JSON.stringify(savedClips));

    toast.success(`저장 완료! ${splitPositions.length + 1}개의 클립으로 나뉩니다.`);
    
    // 콜백 호출
    if (onSave) {
      onSave(splitPositions);
    }
    ...
    <Card className="w-full">
      ...
      <CardContent className="space-y-6">
        ...
        <div className="space-y-4">
          ...
          <div className="bg-muted/30 rounded-lg p-6 border-2 border-dashed border-border hover:border-[#FF6B35]/50 transition-colors">
            <div
              ref={scriptRef}
          ...
          <div className="flex items-start gap-2 text-sm text-muted-foreground bg-[#FF6B35]/10 rounded-lg p-4">
