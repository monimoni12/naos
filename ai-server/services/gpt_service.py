...
**중요: 재료 가격은 한국 대형마트(이마트, 홈플러스, 롯데마트 등) 평균가 기준으로 직접 추정하세요.**

평가 기준:
- price_efficiency (가격 효율성, 35%): 추정 재료 가격 대비 영양가와 양
...
반드시 아래 JSON 형식으로만 응답하세요:
{
    ...
    "estimated_total_price": 추정총재료비(원),
    "summary": "한줄 평가"
}"""
                },
                {
...
반드시 아래 JSON 형식으로만 응답하세요:
{
    ...
    def analyze_recipe_full(self, recipe_data: dict) -> dict:
        """
        가성비 + 영양 정보 통합 분석 (API 호출 1회로 처리)
        ...
        """
        prompt = self._build_full_analysis_prompt(recipe_data)
        ...
        response = self.client.chat.completions.create(
            ...
            messages=[
                {
                    "role": "system",
                    "content": """당신은 요리 레시피 분석 전문가입니다.

주어진 레시피를 분석하여 가성비 점수와 영양 정보를 함께 제공하세요.

**재료 가격은 한국 대형마트(이마트, 홈플러스, 롯데마트 등) 평균가 기준으로 직접 추정하세요.**
...
반드시 아래 JSON 형식으로만 응답하세요:
{
    ...
    "nutrition": {
        ...
        if 'cost_efficiency' in result:
            result['cost_efficiency'] = self._normalize_score(result['cost_efficiency'])
...
이 레시피의 가성비를 분석해주세요. 재료 가격은 한국 대형마트 평균가로 추정해주세요."""
    
    def _build_full_analysis_prompt(self, recipe_data: dict) -> str:
        """통합 분석용 프롬프트 생성"""
        title = recipe_data.get('title', '레시피')
        ingredients = recipe_data.get('ingredients', [])
        cook_time = recipe_data.get('cook_time_min', 0)
