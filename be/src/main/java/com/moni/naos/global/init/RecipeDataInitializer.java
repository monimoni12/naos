 ...
 * 
 * ⭐ 카테고리 (UI와 일치):
 * - 저탄수화물: 체중 감량용 저칼로리 레시피
 * - 고단백: 벌크업/체중 증가용 고단백 레시피
 * - 반찬: 균형 잡힌 식단
 * - 간식: 다이어트 간식
 */
@Slf4j
...
public class RecipeDataInitializer implements CommandLineRunner {
    ...
    public void run(String... args) {
        ...
        createRecipe(seedUser, "저탄수화물", 14,
        ...
        createRecipe(seedUser, "저탄수화물", 15,
        ...
        log.info("저탄수화물 레시피 5개 생성 완료");

        // ==================== 고단백 (구 GAIN) - 4개 ====================

        createRecipe(seedUser, "고단백", 3,
        ...
        log.info("고단백 레시피 4개 생성 완료");

        // ==================== 반찬 (구 BALANCE) - 4개 ====================

        createRecipe(seedUser, "반찬", 6,
        ...
        log.info("반찬 레시피 4개 생성 완료");

        // ==================== 간식 (구 SNACK) - 5개 ====================

        createRecipe(seedUser, "간식", 1,
    ...
    /**
     * ⭐ 카테고리별 가성비 점수 자동 생성 (테스트용)
     * - 저탄수화물: 55~90점 (저칼로리, 다양한 재료)
     * - 고단백: 45~75점 (고단백, 재료비 높음)
     * - 반찬: 50~85점 (균형 잡힌 식단)
     * - 간식: 60~95점 (간단한 재료)
     */
    private Double generateCostEfficiencyScore(String category) {
        ...
        double score = switch (category) {
            case "저탄수화물" -> 55.0 + random.nextDouble() * 35;    // 55~90
            case "고단백" -> 45.0 + random.nextDouble() * 30;        // 45~75
            case "반찬" -> 50.0 + random.nextDouble() * 35;          // 50~85
            case "간식" -> 60.0 + random.nextDouble() * 35;          // 60~95
