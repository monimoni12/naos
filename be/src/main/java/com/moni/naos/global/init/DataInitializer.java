...
@Slf4j
@Component
@Order(1)  // RecipeDataInitializer(@Order(3))보다 먼저 실행
...
public class DataInitializer implements CommandLineRunner {
    ...
    public void run(String... args) {
        ...
        initRewardPolicies();
        initBadges();
        initSeedUser();  // 시드 유저 생성
    }

    // ==================== Seed User ====================
    private void initSeedUser() {
        if (userRepository.findByEmail("seed@naos.com").isPresent()) return;
        ...
        User seedUser = User.builder()
                .email("seed@naos.com")
                .passwordHash(passwordEncoder.encode("seed1234!"))
                .role(userRole)
                .build();
        userRepository.save(seedUser);

        Profile seedProfile = Profile.builder()
                .user(seedUser)
                .username("naos_official")
                .fullName("NAOS 공식")
                .bio("건강한 레시피를 공유합니다 🥗")
                .build();
        profileRepository.save(seedProfile);

        log.info("✅ 시드 유저 생성 완료 (seed@naos.com)");
    }
